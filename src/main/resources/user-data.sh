...
prepare_machine_generic () {
    ...
    mkdir /opt/openbaton
    #Installation of pip
    wget http://openbaton/openbaton/get-pip.py
...
configure_ems () {
    wget http://openbaton/openbaton/ems.py -O /usr/local/lib/python3.9/dist-packages/ems/ems.py
    wget http://openbaton/openbaton/receiver.py -O /usr/local/lib/python3.9/dist-packages/ems/receiver.py
    mkdir -p /opt/openbaton/scripts
    wget http://openbaton/openbaton/scripts --directory-prefix=/opt/openbaton/scripts/ -r -np -nd --include openbaton/scripts -A sh,py
